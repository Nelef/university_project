...
class Drive_save_activity : AppCompatActivity() {
    ...
    public override fun onActivityResult(requestCode: Int, resultCode: Int, resultData: Intent?) {
        when (requestCode) {
            Drive_save_activity.Companion.REQUEST_CODE_SIGN_IN -> if (resultCode == RESULT_OK && resultData != null) {
                handleSignInResult(resultData)
            }
            Drive_save_activity.Companion.REQUEST_CODE_OPEN_DOCUMENT -> if (resultCode == RESULT_OK && resultData != null) {
    ...
    private fun requestSignIn() {
        Log.d(Drive_save_activity.Companion.TAG, "Requesting sign-in")
        ...
        startActivityForResult(client.signInIntent, Drive_save_activity.Companion.REQUEST_CODE_SIGN_IN)
    }
    ...
    private fun handleSignInResult(result: Intent) {
        GoogleSignIn.getSignedInAccountFromIntent(result)
            .addOnSuccessListener { googleAccount: GoogleSignInAccount ->
                Log.d(Drive_save_activity.Companion.TAG, "Signed in as " + googleAccount.email)
            ...
            .addOnFailureListener { exception: Exception? -> Log.e(Drive_save_activity.Companion.TAG, "Unable to sign in.", exception) }
    }
    ...
    private fun openFileFromFilePicker(uri: Uri) {
        if (mDriveServiceHelper != null) {
            Log.d(Drive_save_activity.Companion.TAG, "Opening " + uri.path)
            mDriveServiceHelper!!.openFileUsingStorageAccessFramework(contentResolver, uri)
                ...
                .addOnFailureListener { exception: Exception? -> Log.e(Drive_save_activity.Companion.TAG, "Unable to open file from picker.", exception) }
        }
    }
    ...
    private fun createFile() {
        if (mDriveServiceHelper != null) {
            Log.d(Drive_save_activity.Companion.TAG, "Creating a file.")
            mDriveServiceHelper!!.createFile()
                .addOnSuccessListener { fileId: String -> readFile(fileId) }
                .addOnFailureListener { exception: Exception? -> Log.e(Drive_save_activity.Companion.TAG, "Couldn't create file.", exception) }
        }
    }
    ...
    private fun readFile(fileId: String) {
        if (mDriveServiceHelper != null) {
            Log.d(Drive_save_activity.Companion.TAG, "Reading file $fileId")
            mDriveServiceHelper!!.readFile(fileId)
                ...
                .addOnFailureListener { exception: Exception? -> Log.e(Drive_save_activity.Companion.TAG, "Couldn't read file.", exception) }
        }
    }
    ...
    private fun saveFile() {
        if (mDriveServiceHelper != null && mOpenFileId != null) {
            Log.d(Drive_save_activity.Companion.TAG, "Saving $mOpenFileId")
            val fileName = mFileTitleEditText!!.text.toString()
            val fileContent = mDocContentEditText!!.text.toString()
            mDriveServiceHelper!!.saveFile(mOpenFileId, fileName, fileContent)
                .addOnFailureListener { exception: Exception? -> Log.e(Drive_save_activity.Companion.TAG, "Unable to save file via REST.", exception) }
        }
    }
    ...
    private fun query() {
        if (mDriveServiceHelper != null) {
            Log.d(Drive_save_activity.Companion.TAG, "Querying for files.")
            mDriveServiceHelper!!.queryFiles()
                ...
                .addOnFailureListener { exception: Exception? -> Log.e(Drive_save_activity.Companion.TAG, "Unable to query files.", exception) }
        }
    }
    ...
    companion object {
        private const val TAG = "Drive_save_activity"
